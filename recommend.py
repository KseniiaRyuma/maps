...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    # distances = [distance(location, pointer) for pointer in centroids]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        restautrants_clusters = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(cluster) for cluster in restautrants_clusters]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    #return [find_predictor(user, reviewed, x) for x in feature_fns ]
    values = [find_predictor(user, reviewed, feature) for feature in feature_fns]
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    return dict([(restaurant_name(restaurant), user_rating(user, (restaurant_name(restaurant)))) \
                     if restaurant in reviewed else (restaurant_name(restaurant), predictor(restaurant)) for restaurant in restaurants])
    # END Question 9
